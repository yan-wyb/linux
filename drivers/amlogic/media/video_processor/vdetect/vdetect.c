/*
 * drivers/amlogic/media/video_processor/vdetect/vdetect.c
 *
...
#define VDETECT_VERSION \
	KERNEL_VERSION(VDETECT_MAJOR_VERSION, VDETECT_MINOR_VERSION,\
		       VDETECT_RELEASE)
...
static int vdetect_release(struct inode *inode, struct file *file)
{
	...
	dev = video_drvdata(file);
	if (vdetect_id != NULL && (*vdetect_id) != -1) {
		vdetect_print(dev->inst, PRINT_PATHINFO,
			      "%p: vdetect_id leak detected, release it: %d\n",
			      file, *vdetect_id);
...
static struct class_attribute vdetect_class_attrs[] = {
	__ATTR(vdetect_debug, 0664,
	       vdetect_debug_show,
	       vdetect_debug_store),
	__ATTR(vdetect_dump, 0664,
	       vdetect_dump_show,
	       vdetect_dump_store),
	__ATTR(vdetect_use_thread, 0664,
	       vdetect_use_thread_show,
	       vdetect_use_thread_store),
	__ATTR_NULL
};

static struct class vdetect_class = {
	.name = "vdetect",
	.class_attrs = vdetect_class_attrs,
};
...
MODULE_DESCRIPTION("Video Technology Magazine video frame detect Capture Boad");
MODULE_AUTHOR("Amlogic, Renjiang Han<renjiang.han@amlogic.com>");
MODULE_LICENSE("GPL");
MODULE_VERSION("1.0");
