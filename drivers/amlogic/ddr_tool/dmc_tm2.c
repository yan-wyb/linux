...
static size_t tm2_dmc_dump_reg(char *buf)
{
	...
	for (i = 0; i < 2; i++) {
		val = dmc_rw(DMC_PROT0_RANGE + (i * 12), 0, DMC_READ);
		sz += sprintf(buf + sz, "DMC_PROT%zu_RANGE:%lx\n", i, val);
		val = dmc_rw(DMC_PROT0_CTRL + (i * 12), 0, DMC_READ);
		sz += sprintf(buf + sz, "DMC_PROT%zu_CTRL:%lx\n", i, val);
		val = dmc_rw(DMC_PROT0_CTRL1 + (i * 12), 0, DMC_READ);
		sz += sprintf(buf + sz, "DMC_PROT%zu_CTRL1:%lx\n", i, val);
	}
	for (i = 0; i < 4; i++) {
		val = dmc_rw(DMC_PROT_VIO_0 + (i << 2), 0, DMC_READ);
		sz += sprintf(buf + sz, "DMC_PROT_VIO_%zu:%lx\n", i, val);
	}
	val = dmc_rw(DMC_PROT_IRQ_CTRL, 0, DMC_READ);
	sz += sprintf(buf + sz, "DMC_PROT_IRQ_CTRL:%lx\n", val);
	val = dmc_rw(DMC_IRQ_STS, 0, DMC_READ);
...
static void check_violation(struct dmc_monitor *mon, void *data)
{
	...
	for (i = 1; i < 4; i += 2) {
		status = dmc_rw(DMC_PROT_VIO_0 + (i << 2), 0, DMC_READ);
		if (!(status & (1 << 19)))
			continue;
		addr = dmc_rw(DMC_PROT_VIO_0 + ((i - 1) << 2), 0,
			      DMC_READ);
...
static void tm2_dmc_mon_irq(struct dmc_monitor *mon, void *data)
{
	...
	value = dmc_rw(DMC_IRQ_STS, 0, DMC_READ);
	if (in_interrupt()) {
	...
	value |= 0x04;		/* en */
	dmc_rw(DMC_PROT_IRQ_CTRL, value, DMC_WRITE);
}
...
static int tm2_dmc_mon_set(struct dmc_monitor *mon)
{
	...
	dmc_rw(DMC_PROT0_CTRL, mon->device | 1 << 24, DMC_WRITE);
...
void tm2_dmc_mon_disable(struct dmc_monitor *mon)
{
	...
	dmc_rw(DMC_PROT_IRQ_CTRL, 0, DMC_WRITE);
	mon->device     = 0;
	mon->addr_start = 0;
	mon->addr_end   = 0;
}
